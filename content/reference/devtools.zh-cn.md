...
标题栏携带三个幽灵图标按钮 — **⌖** (拾取)、**⟳** (刷新)、**⚠** (审计) — 以及三个计数徽章：总实体数、可交互实体数 (**⚡**) 和审计发现数 (**⚠**)。一个 `Tabs` 栏将工具分为 **Tree · Info · Audit · A11y · Log · ⚙**，加上每个已注册的[插件检查器](/reference/devtools-extend/#cha-jian-xie-yi)对应一个标签页。性能条固定在底部。
...
- **拾取模式**：点击 **⌖**，然后点击页面上的任意位置。检查器使用与 Scene 用于指针输入相同的遍历顺序（以及相同的接受规则），将点击解析到该点下最深的实体 —— 只有实体自身的形状接受该点的位置才是可拾取的，与引擎完全一致，因此粒子和其他非交互式实体绝不会成为错误的所有者。
- **选择高亮**：所选实体的几何形状作为轮廓绘制在*宿主*场景的覆盖层上，因此你能准确看到相对于实时渲染选中了什么。默认情况下它绘制布局盒；`panel.setHighlightLayers()` 可将其切换为七个[高亮几何层](/reference/devtools-inspect/#gao-liang-ji-he)中的任意一个 — 包括 `'hit'`，它对实体的真实点击区域进行采样而非其包围盒。
...
- **A11y 标签页**：选中实体的投影角色、可访问名称及其来源、tab 索引、阅读顺序位置、画布对比 DOM 边界框 — 加上全场景 [a11y 审计](/reference/devtools-audit/#fu-zhu-gong-neng-shen-ji) 发现。
