...
// options: { font: MSDFFont, texture: TexImageSource, fallbackFont?, fontSize? (default 32),
//            color?, lineHeight?, letterSpacing?, maxWidth? (default 1000), maxHeight? (default 1000),
...
```ts
new TextEntity(text: string, atlas: GlyphAtlas, maxWidth: number, fontSize = 24)
...
underlying `LayoutEngine`. `MSDFTextEntity` exposes the full control surface —
`setTextAlign('justify')` and `setHyphenator(fn)` (soft hyphens U+00AD in the
source work without a hyphenator) — because its Canvas2D fallback draws **per
glyph** at each node's computed `x`, so the justification/hyphenation math is
visually honored. `Text`/`RichText` also accept `textAlign: 'justify'` and
`hyphenate` options; they render each wrapped line as coalesced `fillText()`
runs, which honors justification for whole runs.
