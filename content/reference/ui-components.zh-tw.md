...
> 文件版本：**2.20.1**。事實來源：`dist/index.d.ts`（公開表面）和 `packages/ui/src/*`（行為）。
...
**顯示：** 將 `enterMotion` / `exitMotion` 宣告為 `MotionSpec`（`{ props: { opacity: [0, 1], … }, config? }`），元件在掛載到即時場景時會動畫進入，並在 `dismiss()` 時動畫退出 — 後者會延遲其自身移除直到退出動畫解析。一個共享的實作基於[核心動畫系統](/reference/core-entity/#dong-hua)，取代每個元件手寫的彈簧。在 `prefers-reduced-motion` 下會抑制動畫（保留透明度淡入淡出效果）。
...
這是本套件唯一匯出的文字量測輔助函式。貪婪式的 `wrapLines` 匯出已在 2.20.0 中移除 —— 它的換行與每個元件實際使用的 LayoutEngine 不一致，它預測的行與實際渲染的內容從不匹配 —— 而 `wrapText` 仍是 `TextArea` 的內部工具而非公開 API。對佈局關鍵的換行請直接使用 LayoutEngine 本身；對於預覽與量測，`measureText` 加上你自己的斷行邏輯是受支援的路徑。
...
一個全螢幕暗化背景，帶有包含 `title` 文字和內建「關閉」按鈕的居中 `Card`。卡片在掛載時透過共享的[動畫系統](/reference/core-entity/#dong-hua)以彈簧動畫縮放進入；阻擋底層 `click`/`pointerdown`。使用 `scene.showOverlay(modal)` 顯示。
