...
アニメーションアクセラレータについては、種類ごとの判定がどのドライバーファミリーが拒否したかを示します：一方のアニムカーネルがフレームを拒否し、もう一方がまだWASMを通じてステップ実行していた場合、`reason`は`springs-rejected`または`tweens-rejected`を報告します（作業の半分は実行されたため、`activeThisFrame: true`）。単純な`rejected`は両方が拒否した場合のためのものです。

`faulted`はまさに`reason === 'rejected'`（種類ごとの判定を含む）であり、`auditAccelerators`はそれだけを報告します。これは意図的です：閉じたままのゲートはシステムが意図どおり動いていることであり、それを報告すると監査を無視するように訓練されてしまいます。健全なシーンも完全にJSなシーンも、どちらもきれいに監査されます。
