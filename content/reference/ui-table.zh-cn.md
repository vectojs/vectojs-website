...
<figure class="sandbox component-demo">
  ...
  <iframe src="/sandbox/ui/component.html?name=table&v=core-1.39.0-ui-2.20.1" class="sandbox-frame component-demo-frame-tall" loading="eager" title="Table live demo" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
...
更改总宽度，按比例缩放各列，并重新布局（`2.11.0+`）。请使用它而不是直接赋值 `width`——单独赋值是不够的：`colWidths` 在**构造函数中解析一次**，使用的是当时给定的宽度，而每个单元格的换行宽度、位置和对齐方式都派生自那些**每列**的数值，而不是派生自 `width`。因此，一个重新赋值了 `width` 的表格会以新尺寸绘制其外框，而其单元格仍按旧尺寸布局。

各列保持其相对比例，因此显式的 `colWidths` 比例能在调整尺寸后存活，而不会在第一次调用时被等分重置。宽度未变时它是空操作，最小值被限制为 1，并返回 `this`。
