...
請將 `cursor` 視為不透明。來源應套用 `limit`、遵守 `direction`、將提供的放棄訊號傳遞給下游工作，並在存在另一個分頁時返回 `nextCursor` 加上 `hasMore`。`total` 是可選的，描述該節點擴充可用的總事實數，而不只是目前的分頁。

`MemoryDataSource` 為測試和小型記憶體圖形實作此合約。其游標是十進位偏移，鄰居查詢是 `O(degree)`，無效的游標會拋出錯誤。
...
使用 `getExpansionState(id)` 讀取防禦性副本。`loaded` 是該擴充中報告的已接受分頁事實數量。`partial` 表示有另一個分頁可用；呼叫 `expand(id)` 會從其儲存的游標繼續。
...
`KnowledgeGraphModelOptions.layout` 接受來自 `@vectojs/graph3d` 的 XYZ `GraphLayout` 合約。提供時，每次具體化重建都會呼叫 `layout.setGraph()`，按節點 ID 保留有限的 XYZ 位置作為暖啟動，並在載入分頁後、當佈局公開 `reheat()` 時重新加熱。

在需要保留最新佈局座標的外部操作之前呼叫 `captureLayoutPositions()`。這個選用合約是三維的：請勿直接傳入來自 `@vectojs/graph-layout` 的 XY `ForceLayout2D`。2D 渲染器可以省略 `layout`，並在 `getGraphData()` 上執行自己的、與渲染器無關的佈局。

## 處置

`dispose()` 中止活躍的請求、處置選用的佈局，並釋放已具體化的狀態。它是冪等的。之後需要活躍模型的方法會拋出 `KnowledgeGraphModel is disposed`；延遲的非同步完成無法重新填入已處置或已被快照取代的狀態。
