...
- **オプションの音声ミックス（0.3.0+）**：外部オーディオファイルを AAC トラックとしてエクスポートにミックスし、動画の長さにトリミングします。指定しない場合、エクスポートは無音のままです。
...
- `-a, --audio`（0.3.0+）：エクスポートにミックスするオーディオファイル（AAC でエンコード）
...
**シーンリセット契約（`0.2.4+`）。** ページ読み込みと `stop()` の間、ページ自身の rAF ループは自由に動くため、実時間状態（イントロトゥイーン、イーズ付きの出現）はキャプチャ開始時点で任意であり、その後のすべてのフレームはその非決定的なベースから出発して初めて決定的でした。エクスポーターは現在、`stop()` の直後、フレーム0がステップまたはキャプチャされる前に、`window.vectoScene` 上のオプションの `reset()` を一度呼び出します。最初の `step(dt)` まで静的にレンダリングするシーンは影響を受けず、`reset()` は不要です。読み込み時の状態を持つシーンが t=0 の表示に戻るには、それを公開することが**必須**です。`reset()` のないシーンはそのままエクスポートされます。
...
API で `audioPath` を（CLI では `-a, --audio <file>` を）渡すと、オーディオトラックをエクスポートにミックスできます：
...
トラックは 192 kbps の AAC でエンコードされ、`-shortest` が動画の長さにトリミングするため、長すぎるオーディオファイルがエクスポートを延長することはありません。存在しないパスやファイルでないパスは、Chromium 起動前のオプション検証で拒否されます。キャンバスキャプチャパイプライン自体は音声を生成しないため、`audioPath` を指定しない限りエクスポートは無音のままです。
