...
コンテンツ投影には2つの独立した層があり、`1.31.0`以降はそれぞれが独自のマージンを持ちます:

- **セマンティック層**（`contentSemanticMargin`）—— このブロックは*何らかの* DOMを持つか？DOMを持つブロックは、そのテキストをネイティブのページ内検索、コピー、スクリーンリーダーの先読みに提供します。
- **インタラクション層**（`contentProjectionMargin`）—— そのブロックの*行単位キャリア*を構築するか？キャリアは選択のための行単位の幾何情報をブラウザに与えます。

分離される前は1つのスカラーが両方を制御していたため、構成は2つしか存在しませんでした。有限の値は画面外のブロックを完全に解放するため画面外テキストが検索できず、`Infinity`はドキュメント内のすべてのキャリアも実体化していました。

2つを分けることで、有用な中間状態が得られます:
...
> [!IMPORTANT]
> `Infinity`は`contentSemanticMargin`では安全ですが、`contentProjectionMargin`では**安全ではありません**。サポート外である理由となるコストは、常駐テキストではなくウィンドウ化されていないキャリア帯から来ています。

インタラクションマージンの外側でセマンティックマージンの内側にあるブロックは、完全なテキストを単一のノードとして投影し、キャリアの子要素を**持ちません**。検索とコピーは可能で、欠けているのは行単位の選択幾何情報だけです。それはいずれにせよビューポートにスクロールインするまで到達できません。

一度だけのコストは知っておく価値があります。常駐層は最初の同期でブロックごとに1つの要素を実体化し、実測で新規作成ノードあたり約13 µs —— 1000ブロックで約47 msです。定常状態は安価です。自身のコンテンツを刻印できるエンティティにより、Sceneは変更のないブロックの再投影を完全にスキップできるからです。つまりこれはドキュメントを開くときのコストであり、フレームごとのコストではありません。
