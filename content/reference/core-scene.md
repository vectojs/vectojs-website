...
| `autoThrottle`            | `boolean`                     | `true`                            | Automatic throttling when the scene is static (no pending transitions, not dirty). Since `1.36.0` the idle floor is **60 fps** by default (set by `idleFPS`); before that it was a hard 2 fps. `false` keeps rendering at `maxFPS` — for scenes that mutate state outside the dirty/transition signals. Also settable live via `scene.autoThrottle`. See "Idle auto-throttle" below. |
| `idleFPS`                 | `number`                      | `60`                              | Since `1.36.0`. Frame-rate floor for an idle `'always'` scene when `autoThrottle` is on. Set `2` to restore the legacy aggressive idle sleep as an explicit choice; `0` keeps the `maxFPS` cadence while idle. Ignored when `autoThrottle: false` or `renderMode: 'onDemand'`. Also settable live via `scene.idleFPS`.                                                               |
...
> leaving the scene on `'always'` and its idle throttle. If you still see
...
scene.autoThrottle: boolean             // idle throttle toggle (default true)
scene.idleFPS: number                  // idle 'always'-mode floor in fps (default 60; 2 = legacy deep sleep)
...
tween. In `'always'` mode with `maxFPS > 0`, a static scene is throttled to the
**idle floor** — **60 fps since `1.36.0`** (set by `idleFPS`), a hard 2 fps
before that — to save battery/GPU. Set `autoThrottle: false` (option or live
`scene.autoThrottle`) to disable the throttle entirely, or `idleFPS: 2` to
restore the aggressive legacy sleep. The `dirty` flag is consumed at the
_start_ of each rendered frame, so a `markDirty()` issued from inside
`update()` survives into the next frame's static check:

> Hand-animated motion (mutating `entity.x` etc. inside a custom `update()`)
> is invisible to the static check unless you report it — drive it through
> [`entity.animate()`](/reference/core-entity/#animation) (which keeps the
> scene non-static while the tween runs), override `hasPendingAnimations()`
> to return `true` while your integrator is running, or call
> `scene.markDirty()` each frame from your `update()` (which re-arms the next
> frame). Otherwise the scene idles down to the throttle floor and your
> motion crawls.
