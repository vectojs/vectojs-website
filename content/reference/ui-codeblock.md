...
<figure class="sandbox component-demo">
  ...
  <iframe src="/sandbox/ui/component.html?name=codeblock&v=core-1.39.0-ui-2.20.1" class="sandbox-frame component-demo-frame-tall" loading="eager" title="CodeBlock live demo" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
...
Long lines overflow rather than wrap, so wide blocks are horizontally scrollable.
`maxScrollX` is memoized against the grid that produced it — `scrollX` is read
every synced frame, so the cache is what keeps a long block from paying an
O(lines) scan per frame.
