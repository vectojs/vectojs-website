...
`animate()` はトゥイーンをキューに入れます；複数回の呼び出しは**逐次連鎖**します。数値プロパティのみが補間されます；イージングは固定の ease-out（`p * (2 - p)`）です。実行中の `animate()` はシーンを非静的状態に保ち（アイドルスロットルを回避、[`Scene`](/reference/core-scene/#rendermode-maxfps-oyobiaidoruzi-dong-surotutoru) を参照）、モーションが落ち着くまでa11y同期をフリーズします。
