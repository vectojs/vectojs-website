...
La version 0.2.1 possède une seule implémentation, le `ForceLayout2D` en TypeScript. Il n'y a pas de build WASM, de backend alternatif, ni d'option `backend` en 0.2.1. WASM reste une option future conditionnée par des mesures ; les comparaisons inter-dimensionnelles actuelles dans les navigateurs ne constituent pas une preuve directe qu'un backend WASM aiderait.
...
`step()` est synchrone. Il renvoie `true` tant que la simulation reste active et `false` après qu'elle a refroidi en dessous de `alphaMin` (ou lorsque le graphe est vide). La valeur de retour indique si la physique a besoin d'un autre tic ; elle ne dit rien sur le fait que votre application doive continuer à rendre pour le mouvement de caméra, la saisie, ou d'autres animations. `alphaDecay: 0` désactive le refroidissement, donc une simulation non vide ne se stabilise pas d'elle-même.
...
| `repulsionDistanceMax` | `Infinity` | Distance maximale à laquelle les nœuds se repoussent. `0` désactive la répulsion ; les valeurs non finies désactivent la coupure.            |
| `alphaDecay`           |   `0.0228` | Décroissance de la température par tic, limitée à `[0, 1]`.                                                                                  |
...
- Les liens parallèles nécessitent des IDs distincts de type chaîne ou nombre fini.
- Rejouer un lien identifié est ignoré.
- Les liens avec une extrémité inconnue ou la même source et cible sont ignorés.
- Un ID de lien optionnel malformé est traité comme absent pour l'identité.

Les liens avec des IDs optionnels malformés entrent tout de même comme liens non identifiés lorsque leurs extrémités sont valides ; les extrémités inconnues et les auto-liens n'entrent pas dans les tableaux de forces. Des données de lien malformées ne rendent pas les positions non finies. `removeNodes(ids)` supprime les nœuds correspondants et chaque lien incident, compacte l'état des survivants, recalcule le biais de degré et réchauffe lorsque quelque chose a été supprimé. Les IDs inconnus et un itérable vide sont sans effet.
...
À l'exécution, `setNodePin(index, { x?, y? })` n'épingle que les axes fournis, met immédiatement à jour ces coordonnées en direct, et efface leur vélocité. `clearNodePin(index, { x?, y? })` libère les axes sélectionnés tout en préservant l'autre axe ; omettre l'objet d'axes libère les deux. `pinNode(index, x, y)` et `unpinNode(index)` restent des méthodes de commodité à deux axes. Les index invalides sont ignorés. Ces appels ne réchauffent pas automatiquement, donc appelez `reheat()` après les opérations interactives d'épinglage ou de désépinglage.
...
function onDragStart(node, x, y) {
  ...
  layout.setNodePin(index, { x, y }); // épingle au pointeur
  layout.reheat(0.3); // réveille la simulation UNE FOIS
}

function onDragMove(node, x, y) {
  const index = layout.getNodeIndex(node.id);
  layout.setNodePin(index, { x, y }); // déplace la broche — pas de réchauffement ici
}

function onDragEnd(node) {
  const index = layout.getNodeIndex(node.id);
  layout.clearNodePin(index); // ou gardez-le épinglé pour une broche permanente
}
```
...
Lorsque la collision est activée, le layout construit le quadtree une seconde fois sur les positions prédites et effectue des requêtes de voisinage par rayon. Les voisinages épars et localement bornés sont généralement proches de `O(N log N + K)`, où `K` est le travail de candidats/chevauchements, mais des grappes denses ou de très grands rayons peuvent rendre `K` quadratique. La collision n'hérite pas d'une borne inconditionnelle `O(N log N)` de la répulsion Barnes-Hut.

`setGraph()` est `O(N + E)` en dehors de l'allocation géométrique de capacité et de l'initialisation. `appendGraph()` est proportionnel à l'entrée ajoutée plus un recalcul de biais de degré en `O(N + E)` lorsque des liens sont acceptés. `removeLinks()` ne compacte que le stockage des liens et est `O(E + R)` lorsque les demandes sont des liens complets, ou `O(E + RE)` dans le pire cas lorsque `R` IDs nus scannent chacun tous les liens. `updateLinks()` est `O(E + U)` pour `U` mises à jour. Le stockage croît géométriquement, donc la plupart des petits ajouts réutilisent la capacité ; une frontière de croissance copie les tableaux typés existants en `O(N + E)`. `removeNodes()` compacte les nœuds et les liens et recalcule le biais en `O(N + E)`. La suppression ne réduit pas la capacité.
...
Les liens utilisent des IDs d'extrémités plutôt que des objets d'extrémités mutés par d3. Les accesseurs d'option reçoivent le `GraphNode` ou `GraphLink` d'origine et un index d'insertion, puis sont mis en cache. Il n'y a pas de registre de forces personnalisées en 0.2.1 ; si votre layout d3 dépend de forces personnalisées ou de setters de forces en direct, conservez d3-force ou recréez le layout avec de nouvelles options.
