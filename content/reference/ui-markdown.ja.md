...
- 開いている [`createStream()`](#sutorimingu) のライターは影響を受けず、追記を続けます;
- 字句解析はやり直されません。
...
[`onStable`](#wansiyotutowan-liao-onstable) コールバックから呼ぶと、`setContent()` と同じ理由で例外を投げます: そのコールバックは、それが無効化しようとしているコミットの内部で走っているからです。
...
### ワンショット完了: `onStable` {#wansiyotutowan-liao-onstable}
