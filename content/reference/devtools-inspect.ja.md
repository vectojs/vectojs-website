...
`pickInScene`は「このピクセルを所有しているエンティティはどれか」を問うときに使いたい関数です。**オーバーレイツリーを先に**チェックし、次にメインツリーをチェックするため、開いたモーダルはその背後にあるコンテンツに正しく優先します。`findEntityAt`はその下にある単一ツリーの基本プリミティブです：子を逆順で、最深優先で走査するため、最も上に描画されたヒットを返し、エンジンの `HitTester` に一致し独自のフォールバックを持たないため、エンティティはそのエンティティ自身の `isPointInside` がその点を受け入れる場合にのみ勝ちます。したがって装飾用やクリップされたエンティティは、実際のクリックとまったく同じように、背後にあるものへ解決されます。
