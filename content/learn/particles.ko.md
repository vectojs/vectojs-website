...
**각 파티클에 `interactive = true`를 설정하지 마세요.** 그러면 엔티티당 하나의 실제 DOM 요소가 의미 계층에 투영되며, 개수가 증가할수록 엔티티당 비용이 악화됩니다 — RTX 4060 노트북에서 측정한 결과, 20,000개의 개별적으로 상호작용하는 이동 엔티티는 Chrome에서 715ms/프레임, Firefox에서 2,737ms/프레임이 소요되었습니다.[비용 표](/learn/accessibility/#인터랙티브-엔티티-수에-따라-비용이-초선형적으로-증가함)를 참조하세요.
