...
파티클 필드는 장식적입니다: 개별 파티클은 공지할 가치가 있는 의미를 전달하지 않으며, 아무도 devtools에서 검사하거나 텍스트로 선택하지 않습니다. 필드를 단일 객체로 취급하세요.

**각 파티클에 `interactive = true`를 설정하지 마세요.** 그러면 엔티티당 하나의 실제 DOM 요소가 의미 계층에 투영되며, 개수가 증가할수록 엔티티당 비용이 악화됩니다 — RTX 4060 노트북에서 측정한 결과, 20,000개의 개별적으로 상호작용하는 이동 엔티티는 Chrome에서 715ms/프레임, Firefox에서 2,737ms/프레임이 소요되었습니다.[비용 표](/learn/accessibility/#inteoraegtibeu-entiti-sue-ddara-biyongi-coseonhyeongjeogeuro-jeunggaham)를 참조하세요.

대신:

- **필드를 한 번만 레이블링하세요.** `ComputeParticleEntity`(또는 래퍼)에 전체 효과를 설명하는 `role`과 `aria-label`을 반환하는 단일 `getA11yAttributes()`를 제공하세요. 하나의 노드, 일정한 비용입니다.
- **투영 없이 히트 테스트하세요.** `scene.findEntityAt(x, y)`는 `interactive` 여부와 관계없이 엔티티를 해결하므로 포인터 상호작용에 투영된 요소가 필요하지 않습니다. `pointerEvents: true`는 커서 좌표를 시뮬레이션에 공급하며 의미 계층과 독립적입니다.
- **효과가 순전히 장식적인 경우 그렇게 말하세요.** 투영되지 않은 상태로 두는 것이 올바른 답이며, 장식적 DOM 요소의 `aria-hidden`과 동일합니다 — 그러나 효과가 전달하는 *정보*가 텍스트로도 제공되는지 확인하세요.
