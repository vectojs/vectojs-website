...
ツリー、グリッド、メニュー、ラジオグループまたはタブリストは、すべての子をタブ順序に置いてはいけません。VectoJSは各**可視**な子の上に透明なフォーカス可能なホットスポットをプールし、その子のロールと状態を持たせ、正確に1つに`tabIndex: 0` —— **ルービング tabindex** —— を与えます。親は矢印キーハンドラを所有し、ストップを移動します。各コンポーネントのキーについては上記のテーブルを参照し、独自に構築する場合は[複合ウィジェット](/reference/core-a11y/#複合ウィジェットロービング-tabindex)を参照してください。
...
- **到達可能なものだけを投影する。** 仮想化された`TreeView`/`Table`が使用するプーリングパターンは、データセット全体ではなく可視行にホットスポットプールのサイズを合わせるため、投影はO(ビューポート)に留まります。[複合ウィジェット](#複合ウィジェット1つのタブストップ内部で矢印キー)を参照してください。
...
エンティティごとの `a11yProjection` モード（`'eager' | 'onDemand' | 'never'`、デフォルトは `'eager'`）は、エンティティのシャドウノードがいつ具体化されるかを制御します。測定値とAPIは[`core-a11y`](/reference/core-a11y/#projection-cost-at-high-entity-counts-1300)に文書化されています。「スクリーンリーダーが存在するか」をキーにできないことに注意してください —— それは設計上（W3C TAG設計原則2.7）意図的に検出不可能であり、AOM仮想アクセシビリティノードはすべてのエンジンでプライバシー上の理由からブロックされています。
