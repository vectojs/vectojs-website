...
## 複合ウィジェット：1つのタブストップ、内部で矢印キー {#fu-he-uizietuto-1tunotabusutotupu-nei-bu-deshi-yin-ki}

ツリー、グリッド、メニュー、ラジオグループまたはタブリストは、すべての子をタブ順序に置いてはいけません。VectoJSは各**可視**な子の上に透明なフォーカス可能なホットスポットをプールし、その子のロールと状態を持たせ、正確に1つに`tabIndex: 0` —— **ルービング tabindex** —— を与えます。親は矢印キーハンドラを所有し、ストップを移動します。各コンポーネントのキーについては上記のテーブルを参照し、独自に構築する場合は[複合ウィジェット](/reference/core-a11y/#fu-he-uizietuto-robingu-tabindex)を参照してください。
...
スロットリングは古さとコストを交換するものであり、同期ごとの作業量を減らすわけではありません。問題がエンティティの**数**であって同期頻度ではない場合は、次のセクションを参照してください。

## コストはインタラクティブエンティティ数に対して超線形に増加する

プロジェクションはUIには安価ですが、多数の要素には高価です。実ハードウェアで測定（RTX 4060ラップトップ、エンティティは毎フレーム移動、各エンティティに1つの投影要素）：

| インタラクティブエンティティ数 | Chrome 1フレームあたり | Firefox 1フレームあたり |
| ------------------------------ | ---------------------- | ----------------------- |
| 1,000                          | 6.4ms                  | 7.4ms                   |
| 5,000                          | 59.5ms                 | 114ms                   |
| 20,000                         | 715ms                  | 2737ms                  |

1,000から20,000への増加で、Chromeでは6.4→35.7µs、Firefoxでは7.4→136.9µsと、エンティティあたりのコストは数が増えるにつれて**悪化**します。なぜなら、コストは要素ごとのDOM書き込みと読み取り順序のソート、そしてブラウザ自身のアクセシビリティツリー再構築であり、これらすべてが要素数とともに劣化するからです。ツリーウォーク自体は無視できる程度です（約0.005µs/エンティティ）。

実用的なルール：`interactive = true`はユーザーが操作するものに対して使います。何千もの装飾的または一時的なオブジェクトをヒットテスト可能にする方法ではありません。

パーティクルフィールド、弾幕レイヤー、スプライト群には、次のいずれかを優先してください：

- **コンテナを投影し、メンバーは投影しない。** レイヤー全体に対して1つのインタラクティブエンティティを使い、`aria-label`でまとめて説明し（例：「5,000パーティクル」）、`scene.findEntityAt(x, y)`を介して自分でポインタ入力を処理します —— これはエンティティが`interactive`かどうかに関係なく解決されるため、ヒットテストに投影は必要ありません。
- **到達可能なものだけを投影する。** 仮想化された`TreeView`/`Table`が使用するプーリングパターンは、データセット全体ではなく可視行にホットスポットプールのサイズを合わせるため、投影はO(ビューポート)に留まります。[複合ウィジェット](#fu-he-uizietuto-1tunotabusutotupu-nei-bu-deshi-yin-ki)を参照してください。
- **エンティティが操作可能でなくなったら`scene.detachA11y(entity)`を呼び出す。** リーク回避として別の場所で文書化されていますが、同様にコストレバーでもあります：毎フレームの同期は作成と更新を行いますが、プルーニングは行いません。

エンティティごとの `a11yProjection` モード（`'eager' | 'onDemand' | 'never'`、デフォルトは `'eager'`）は、エンティティのシャドウノードがいつ具体化されるかを制御します。測定値とAPIは[`core-a11y`](/reference/core-a11y/#gao-ienteiteishu-niokerutou-ying-kosuto-1-30-0)に文書化されています。「スクリーンリーダーが存在するか」をキーにできないことに注意してください —— それは設計上（W3C TAG設計原則2.7）意図的に検出不可能であり、AOM仮想アクセシビリティノードはすべてのエンジンでプライバシー上の理由からブロックされています。
