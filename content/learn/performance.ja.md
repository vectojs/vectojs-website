...
エンティティ単位のカリングにも、ツリーを歩くコストはかかります。**キャンバス自体**が完全にビューの外へスクロールした場合——ダッシュボードのタブ、フォールドより下にあるチャートなど——`IntersectionObserver`がrAFループを完全に一時停止し、再び入ったときに再開します。そのため、誰にも見えていないシーンは、フレームごとに完全な更新/レンダリングを行う代わりに、まったくコストがかかりません。オプトインは不要です。（SSR/jsdomなど`IntersectionObserver`が利用できない環境では、シーンは常に画面内として扱われます。）

### `dt`は100msに制限される

バックグラウンドに回ったタブ、デバッガの一時停止、長いGCの後では、実際の経過時間は数秒になることがあります。その生の値を積分に与えると物理演算やトゥイーンがテレポートしてしまうため、フレームデルタは`MAX_FRAME_DT`（100ms）で上限が設けられています。`update(dt)`で自分で`dt`を積分する場合も、その値を超えることは決してありません。
...
### 3. エンティティの海のインタラクション（$O(N^2)$計算量の破局） {#3-enteiteinohai-nointarakusiyon-o-n-2-ji-suan-liang-nopo-ju}
...
## 実際のパフォーマンスの測定 {#shi-ji-nopahuomansunoce-ding}
...
FPSは垂直同期によって制限されるため、**飽和**します —— 飽和した数値は回帰も改善も隠蔽します。私たち自身の測定からの実際の例：あるシーンは59 FPSを報告していましたが、17msのフレームのうち3.4msしか仕事をしておらず、各フレームの約80%をアイドル状態で過ごしていました。単に60Hzの垂直同期を交渉していただけです。その59という数字はコードについて何も語っていません。

診断にとって重要な帰結：**FPSが制限されている場合、「Xを変えてもFPSが動かなかった」というのは何も証明しません。** 変更前と変更後の両方が快適にフレーム予算内に収まっている可能性があります。

代わりに以下を測定してください：

- **フレーム時間のパーセンタイル**（p50/p99）、平均ではありません。高リフレッシュレートディスプレイでは、フレーム時間は垂直同期によって1x/2x/3x間隔のバケツに量子化され、その間には何もないため、平均は決して発生しない値を表します。
- **予算内のフレームの割合** —— モーションがロックされていると感じるかどうかを決定する数値。240Hzでは予算は4.17ms、60Hzでは16.67msです。
- **各フェーズのコストを個別に**（レイアウト、JSバッチ処理、GPUサブミット）計測し、どの部分に取り組むべきかを把握します。

### GPU時間の帰属には`gl.finish()`が必要

WebGL呼び出しは非同期です。drawや`flush()`を`performance.now()`でラップすると、**キュー挿入**時間が測定され、GPU作業時間は測定されません —— 私たちの測定では両者は最大5倍乖離しています。サブミットコストを正直に帰属させるには、作業を行った後にパイプラインを強制的に排出します：

```typescript
const t0 = performance.now();
drawEverything();
gl.finish(); // フレームをシリアライズ；これなしでは数値は無意味
const submitMs = performance.now() - t0;
```

`EXT_disjoint_timer_query_webgl2`はより良いツールのように見えますが、実際には信頼できません：Firefoxは一般的にそれを公開せず、Chromeでは存在することが多いものの、使用可能なサンプルを返しません（毎回利用不可または分離を報告）。それに基づいて測定戦略を構築しないでください。

### NodeやBunではなく、ブラウザでベンチマークする

サーバーランタイムは、ユーザーに面するものには適切なツールではありません：GPUなし、コンポジターなし、DPRなし、異なるJITウォームアップとタイマー解像度。これらは**原因の切り分け**には有用ですが（私たちの最適化の1つはNodeプローブによって発見されました）、引用する数値を生成するのには適していません。**Bun/JSCで12.4xと測定された変更が、実際のブラウザでは3.2–4.7x**であり、約3倍楽観的でした。

両方のエンジンで引用してください。V8とSpiderMonkeyは大きく異なり、単一エンジンの数値は繰り返し誤解を招いてきました。

### 実用的なチェックリスト

1. 実際のGPUハードウェア上の実際のブラウザで実行します。
2. Nの中央値を報告します（7は合理的なデフォルト）、シナリオを正確に命名します。
3. ブラウザ+バージョン、CPU/GPU、ビューポートCSSサイズ**とDPR**、エンティティと可視カウント、バックエンドの選択、およびディスプレイのリフレッシュレートを記録します。
4. PRやドキュメントではブラウザ内測定を引用し、ヘッドレス出力は決して引用しません。

カスタムベンチマークでは、`update()`ループでフレーム時間を収集し、パーセンタイルを報告します：
...
class BenchEntity extends Entity {
  update(dt: number, time: number) {
    ...
    if (samples.length === 300) {
      ...
      const budget = 1000 / 60; // 高リフレッシュレートパネルでは1000 / 240を使用
...
`dt`はミリ秒単位です。これはフレーム_間隔_を報告することに注意してください。垂直同期下では量子化されます —— 予算を満たしたかどうかはわかりますが、どれだけの余裕が残っているかはわかりません。余裕を測定するには、制御するフェーズを計時してください。
