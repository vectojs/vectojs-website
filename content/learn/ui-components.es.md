...
| `gap`      | `number`              | `12`        | Espacio vertical entre filas           |
| `accent`   | `string`              | `'#2563eb'` | Relleno del círculo seleccionado       |
| `onChange` | `(v: string) => void` | —           | Callback al cambiar la selección       |

Llama a `radio.selectByValue(val, focusIt?)` en cualquier momento para cambiar programáticamente la selección. Proyecta `role="radiogroup"` con `role="radio"` individual + `aria-checked` en cada opción.
...
| `value`         | `string`               | primera pestaña | Pestaña visible inicialmente             |
| `tabHeight`     | `number`               | `40`            | Altura en píxeles de la fila de la barra |
