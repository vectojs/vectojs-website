...
| 偏移常數                     | 索引 | 欄位       | 備註                                                |
| ---------------------------- | ---- | ---------- | --------------------------------------------------- |
| `PARTICLE_OFFSET_POSITION_X` | 0    | position.x | 當前世界空間 x                                      |
| `PARTICLE_OFFSET_POSITION_Y` | 1    | position.y | 當前世界空間 y                                      |
| `PARTICLE_OFFSET_VELOCITY_X` | 2    | velocity.x |                                                     |
| `PARTICLE_OFFSET_VELOCITY_Y` | 3    | velocity.y |                                                     |
| `PARTICLE_OFFSET_ORIGIN_X`   | 4    | origin.x   | 彈簧靜止/錨點                                       |
| `PARTICLE_OFFSET_ORIGIN_Y`   | 5    | origin.y   |                                                     |
| `PARTICLE_OFFSET_SIZE`       | 6    | size       | 每個粒子的大小覆蓋值                                |
| `PARTICLE_OFFSET_LIFE`       | 7    | life       | `-1` = 永久；`≥0` 以 0.5/s 衰減；`0` = 死亡（跳過） |
...
粒子場是裝飾性的：單個粒子並不攜帶值得宣告的語意，也沒有人會檢查它們或在其中選擇文字。將粒子場視為單一物件。

**不要對每個粒子設定 `interactive = true`。** 這會為每個實體向語意層投影一個真實的DOM元素，而每個實體的成本會隨數量增長而惡化——在RTX 4060筆電上量測，20,000個獨立互動的移動實體在Chrome上耗時715ms/幀，在Firefox上耗時2,737ms/幀。參見[成本表](/learn/accessibility/#cheng-ben-yu-hu-dong-shi-ti-shu-liang-cheng-chao-xian-xing-guan-xi)。

相反：

- **一次性標記粒子場。** 給 `ComputeParticleEntity`（或一個包裝器）一個單一的 `getA11yAttributes()`，回傳一個 `role` 和描述整個效果的 `aria-label`。一個節點，恆定成本。
- **在不投影的情況下進行命中測試。** `scene.findEntityAt(x, y)` 可以解析任何實體，無論其 `interactive` 狀態如何，因此指標互動不需要投影元素。`pointerEvents: true` 向模擬提供游標座標，與語意層無關。
- **如果效果純粹是裝飾性的，那麼就這樣說明。** 不進行投影是正確的做法，相當於裝飾性DOM元素上的 `aria-hidden` —— 但要確保效果傳遞的任何*資訊*也同樣以文字形式提供。
