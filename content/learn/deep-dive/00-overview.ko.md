---
title: '00 — 개요: VectoJS의 열두 보스'
description: 'VectoJS의 열두 딥다이브 보스를 위한 내비게이션 가이드 — 게임 맵, 아키텍처 불변식, 패키지 의존성, 그리고 모든 신입을 위한 읽기 경로.'
order: 20
---

# 00 — 개요: VectoJS의 열두 보스

## 게임

VectoJS는 브라우저 책임을 단일 `<canvas>` 위에 재구현합니다: layout, hit-testing, event dispatch, text shaping, clipping, scrolling, 접근성, 그리고 렌더링 — 모두 retained entity 트리 위의 명시적 연산으로 수행됩니다. 프레임워크를 **열두 보스**가 지키는 게임이라 생각하세요. 각 보스는 DOM이 공짜로 제공하던 서브시스템 하나를 지키며, VectoJS는 이제 그것을 정확히 구현해야 합니다. 순서대로 싸울 필요는 없지만, 싸움을 고르기 전에 맵을 알아야 합니다.

이 문서가 그 맵입니다.

- **여기서 배우는 것**: 한 장의 그림으로 보는 런타임 아키텍처, 패키지 의존성 골격, 각 보스가 위협하는 불변식, 읽기 순서를 고르는 방법, 그리고 기존 `content/learn/*` 및 `content/reference/*` 문서 대비 이 딥다이브 시리즈의 위치.
- **여기서 배우지 않는 것**: 개별 보스의 메커니즘. 각 딥다이브가 자기 보스를 소유합니다. 이 개요는 그곳으로 연결하고, 방향을 잡을 만큼의 최소한만 제공합니다.
...
총 13개 문서: 이 개요(00) plus 보스별 1개씩. 난이도는 코드 양이 아니라 틀리기 쉬운 정도를 의미합니다. "첫 읽기"는 _유용한_ VectoJS 작업에 도달하는 가장 빠른 경로이며, "심화 선행"은 이 보스를 다루기 전에 읽어야 할 다른 보스 하나를 의미합니다.

| #   | 보스 (딥다이브)                                     | 패키지                                                                        | 난이도 | 읽어야 할 대상                            | 심화 선행 | 첫 읽기 대상                            |
| --- | --------------------------------------------------- | ----------------------------------------------------------------------------- | ------ | ----------------------------------------- | --------- | --------------------------------------- |
...
- 02와 06은 00 다음에 꼭 두 개를 골라야 한다면 가장 좋은 "두 번째 읽기"입니다 — 다른 대부분의 보스가 둘 중 하나를 가정합니다.
...
## 모든 보스를 지배하는 세 가지 불변식

각 보스는 이 중 하나를 깰 수 있습니다. 다른 것은 잊어도 이 불변식만은 기억하세요.
...
| 기존 문서                                                                                                                         | 딥다이브 (이 시리즈) | 관계                                                                                                                                                                                                                                                                                                         |
| --------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `content/learn/*` (introduction, runtime-architecture, engine-concepts, text-typography, core-scene, accessibility, streaming 등) | 00–12                | **Learn은 VectoJS를 _사용하는_ 법을 가르칩니다**; 딥다이브는 그 사용법 **안에서 VectoJS가 _어떻게 동작하는지_**를 가르칩니다. learn 챕터를 먼저 읽으면 해당 보스가 더 쉬워집니다. 권장 짝: `text-typography` → 보스 02; `core-scene` + `events` → 보스 06; `accessibility` → 보스 03; `streaming` → 보스 04. |
| `content/reference/*` (core-a11y, core-entities, core-layout, core-text, ui-markdown, three-adapter, graph-layout 등)             | 00–12                | **Reference는 API 진실**입니다 (props, 타입, 서브패스). 딥다이브는 레퍼런스 페이지를 인용하지만 재진술하지 않습니다. 의심스러울 때는 레퍼런스 시그니처가 우선합니다.                                                                                                                                         |
| `forge/findings/*` + `forge/baselines/*`                                                                                          | 각 딥다이브의 부록   | Findings는 **현장 노트**이며, baselines는 **측정된 증거**입니다. 딥다이브는 보스별로 findings를 하나의 서사로 합성하고, 주장을 뒷받침한 `file:line` 항목으로 다시 연결합니다.                                                                                                                                |
...
**"이제 막 합류했어요"** — 00 → 02 (text/layout) → 06 (VMT 생명주기) → 07 (렌더러) → 첫 과제와 가장 가까운 보스. 이틀 오후면 실제 PR을 만들 수 있습니다.

**"기능을 소유하고 있어요"** — 00 → 당신의 보스 → 심화 선행 행 → 해당 `content/learn/*` 챕터 → 그 보스의 `forge/findings/<area>.md`. 리뷰 전에 불변식 섹션을 다시 훑으세요.
...
위에서 경로를 고르세요. 관례적인 다음 읽기는 텍스트를 다룬다면 **보스 01 — 캔버스 네이티브 선택**, 생명주기/이벤트를 다룬다면 **보스 06 — VMT 런타임**입니다 — 둘 다 더 어려운 쌍(02, 08)으로 가는 짧은 진입로입니다.
