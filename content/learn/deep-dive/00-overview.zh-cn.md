---
title: '00 — 总览：VectoJS 的十二个 Boss'
description: 'VectoJS 十二个深度剖析 Boss 的导航指南——游戏地图、架构不变量、包依赖骨架，以及面向各类新人的阅读路径。'
order: 20
---

# 00 — 总览：VectoJS 的十二个 Boss

## 游戏

VectoJS 在单个 `<canvas>` 上重新实现了浏览器本应负责的职能：布局、命中测试、事件分发、文本塑形、裁剪、滚动、无障碍和渲染——全部基于对保留式实体树的显式数学运算。将框架想象成一场拥有 **十二个 Boss** 的游戏，每个 Boss 守卫一个过去由 DOM 免费提供、而 VectoJS 必须做到完全正确的子系统。你不必按顺序挑战它们，但在选择对手之前需要先了解地图。

本文档就是这张地图。

- **你将在此学到**：一张图看懂运行时架构、包依赖骨架、每个 Boss 所威胁的不变量、如何选择阅读顺序，以及这些深度剖析与现有 `content/learn/*` 和 `content/reference/*` 文档的关系。
- **你不会学到**：任何单个 Boss 的具体机制。每个深度剖析都拥有自己的 Boss。本文档负责为你指路，并提供足够的背景让你有方向地抵达。
...
共 13 篇文档：本总览（00）加上每个 Boss 一篇。难度衡量的是“出错的代价”，而非代码量。“首次阅读”指通往_可用_的 VectoJS 工作的最快路径；“深度前置”指在攻克该 Boss 之前应当先读的另一个 Boss。

| #   | Boss（深度剖析）                              | 包                                                                            | 难度 | 适合谁阅读                            | 深度前置 | 首次阅读面向…              |
| --- | --------------------------------------------- | ----------------------------------------------------------------------------- | ---- | ------------------------------------- | -------- | -------------------------- |
...
- 若必须只选两篇作为 00 之后的“第二读”，02 与 06 是最佳选择——大多数其他 Boss 都假设你已掌握其中之一。
...
每个 Boss 都可能破坏其中一条。若你什么都记不住，请记住这些不变量。
...
| 现有文档                                                                                                               | 深度剖析（本系列） | 关系                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content/learn/*`（介绍、运行时架构、引擎概念、文本排版、核心场景、无障碍、流式等）                                    | 00–12              | **Learn 教你如何_使用_ VectoJS**；深度剖析则讲 **VectoJS 在该用法_内部如何工作_**。先读对应的 learn 章节会让相应的 Boss 更容易理解。建议配对：`text-typography` → Boss 02；`core-scene` + `events` → Boss 06；`accessibility` → Boss 03；`streaming` → Boss 04。 |
| `content/reference/*`（core-a11y、core-entities、core-layout、core-text、ui-markdown、three-adapter、graph-layout 等） | 00–12              | **Reference 是 API 真值**（属性、类型、子路径）。深度剖析会引用参考页但不重复它们。如有疑问，以参考签名为准。                                                                                                                                                    |
| `forge/findings/*` + `forge/baselines/*`                                                                               | 每个深度剖析的附录 | Findings 是**实地笔记**；baselines 是**实测证据**。深度剖析将 findings 综合为每个 Boss 的单一叙事，并链回赢得该论断的 `file:line` 条目。                                                                                                                         |
...
**“我刚加入”** — 00 → 02（文本/布局）→ 06（VMT 生命周期）→ 07（渲染器）→ 最贴近你首个任务的 Boss。两个下午，足够提交一个真实 PR。

**“我负责某个功能”** — 00 → 你的 Boss → 它的深度前置行 → 对应的 `content/learn/*` 章节 → 该 Boss 的 `forge/findings/<area>.md`。在评审前再浏览一遍不变量章节。
...
从上面的路径中选择一条。常规的下一步是：若你触及文本则阅读 **Boss 01 — Canvas 原生选区**，若你触及生命周期/事件则阅读 **Boss 06 — VMT 运行时**——两者都是通往更难组合（02、08）的短捷入口。
