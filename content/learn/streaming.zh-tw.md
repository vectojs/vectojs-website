...
1. **詞法分析是漸進式的，以不穩定的尾部為鍵。** 自 0.8.1 起，worker 路徑會從最後一個穩定區塊邊界（`lexAppend`）開始重新進行詞法分析，而不是重新對整個累積的原始碼進行分詞，因此每次區塊的成本追蹤的是發生變化的尾部，而非文件大小。只有使用連結定義區塊或行首 `$$` 數學公式的文件才會回退到整個文件的詞法分析（`lexFull`）。解析在可用的背景 Worker 中運行（在沒有 `Worker` 的環境中回退到同步詞法分析），實體更新會重用所有已完成的區塊。
