...
> **Note on `interactive`:** Most form components (`Button`, `Input`, etc.) set `this.interactive = true` in their constructors. `Text`/`RichText` are deliberately **non-interactive** — their semantic presence is the content projection, and an interactive shadow node above selectable text would intercept the pointer. `Card` is decorative by default — it becomes interactive only when you pass a `label` option.
...
| `gap`      | `number`              | `12`        | Vertical gap between rows              |
| `accent`   | `string`              | `'#2563eb'` | Selected circle fill                   |
| `onChange` | `(v: string) => void` | —           | Callback on selection change           |

Call `radio.selectByValue(val, focusIt?)` at any time to programmatically change the selection. Projects `role="radiogroup"` with individual `role="radio"` + `aria-checked` on each option.
...
| `value`         | `string`               | first tab   | Initially visible tab            |
| `tabHeight`     | `number`               | `40`        | Pixel height of the bar row      |
