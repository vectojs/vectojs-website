...
- 이론: [수학적 기초: a11yRoot](/learn/math-foundations/#2-시맨틱-shadow-dom-a11yroot)
...
- 이론: [수학적 기초: 아핀 변환](/learn/math-foundations/#3-아핀-변환)

## 4. 콜드/핫 레이아웃

텍스트 레이아웃은 비용이 큰 콘텐츠 준비와 반응형 줄바꿈을 분리합니다. 콘텐츠 변경은 콜드 경로를, 너비 변경은 준비된 측정값을 재사용할 수 있습니다.

- 이론: [수학적 기초: 콜드/핫 분할](/learn/math-foundations/#4-coldhot-분할-레이아웃-엔진)
...
- 이론: [수학적 기초: 집합-차분 대수](/learn/math-foundations/#5-텍스트-흐름을-위한-집합-차분-대수)

## 6. 샘플링된 스플라인 히트 테스트

`SplineEntity`는 곡선을 캐시된 선분으로 샘플링하고, 포인터 거리의 제곱을 해당 선분과 비교합니다. 이 방식은 픽셀 읽기를 피하면서 AABB만 사용하는 히트 테스트보다 더 정밀합니다.

- 이론: [수학적 기초: 샘플링된 스플라인 히트 테스트](/learn/math-foundations/#6-샘플링된-스플라인-히트-테스팅)

## 7. 반-암시적 오일러 동역학

중단된 UI 전환은 일회성 CSS 타이머 대신 스프링 계 시스템으로 모델링됩니다. 목표값이 전환 중간에 변경되어도 모션이 연속성을 유지합니다.

- 이론: [수학적 기초: ODE 동역학](/learn/math-foundations/#7-미분-방정식-및-semi-implicit-euler-솔버)
...
- 이론: [수학적 기초: SpatialHashGrid 유틸리티](/learn/math-foundations/#8-spatialhashgrid-유틸리티)
