...
트리, 그리드, 메뉴, 라디오 그룹 또는 탭 목록은 모든 자식을 탭 순서에 넣어서는 안 됩니다. VectoJS는 각 **보이는** 자식 위에 투명한 포커스 가능 핫스팟을 풀링하여 해당 자식의 역할과 상태를 유지하고, 정확히 하나에 `tabIndex: 0` —— **루빙 tabindex** —— 를 부여합니다. 상위가 화살표 키 핸들러를 소유하고 정지를 이동합니다. 위의 표에서 각 구성 요소의 키를 확인하고, 직접 구축하는 경우 [복합 위젯](/reference/core-a11y/#복합-위젯-로빙-tabindex)을 참조하세요.
...
- **도달 가능한 것만 투영하세요.** 가상화된 `TreeView`/`Table`이 사용하는 풀링 패턴은 데이터셋 전체가 아닌 보이는 행에 핫스팟 풀 크기를 맞추므로 투영이 O(뷰포트)로 유지됩니다. [복합 위젯](#복합-위젯-하나의-탭-정지-내부-화살표-키)을 참조하세요.
...
엔티티별 `a11yProjection` 모드(`'eager' | 'onDemand' | 'never'`, 기본값은 `'eager'`)는 엔티티의 섀도 노드가 언제 구체화되는지 제어합니다. 측정값과 API는 [`core-a11y`](/reference/core-a11y/#projection-cost-at-high-entity-counts-1300)에 문서화되어 있습니다. 이는 "스크린 리더가 있는지"를 키로 사용할 수 없음에 유의하세요 — 이는 설계상(W3C TAG 설계 원칙 2.7) 의도적으로 감지 불가능하며, AOM 가상 접근성 노드는 모든 엔진에서 개인정보 보호 이유로 차단되어 있습니다.
