...
- **到達可能なものだけを投影する。** 仮想化された`TreeView`/`Table`が使用するプーリングパターンは、データセット全体ではなく可視行にホットスポットプールのサイズを合わせるため、投影はO(ビューポート)に留まります。[複合ウィジェット](#複合ウィジェット——1つのタブストップ、矢印キーで操作)を参照してください。
