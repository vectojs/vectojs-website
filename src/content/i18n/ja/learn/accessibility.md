...
> エンティティごとの`a11yProjection`モード（`'eager' | 'onDemand' | 'never'`）で、ホバー/フォーカス時にのみノードを実体化するものが設計されていますが、**まだ実装されていません**。これは「スクリーンリーダーが存在するか」をキーにできないことに注意してください —— それは設計上（W3C TAG設計原則2.7）意図的に検出不可能であり、AOM仮想アクセシビリティノードはすべてのエンジンでプライバシー上の理由からブロックされています。
