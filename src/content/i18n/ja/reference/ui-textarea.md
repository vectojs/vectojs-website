...
<figure class="sandbox component-demo">
  ...
  <iframe src="/sandbox/ui/component.html?name=textarea&v=core-1.32.6-ui-2.15.0" class="sandbox-frame component-demo-frame-tall" loading="eager" title="TextArea live demo" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
...
canvas は**ネイティブ要素の** `scrollTop` に追従します（2.10.0+）。ミラーがスクロールの権威であり、ブラウザはすでにそれをスクロール済みであるため、ホイールハンドラーはありません——追加するとジェスチャーが二重に適用されてしまいます。

2.10.0 より前は、canvas のスクロール位置はキャレット駆動のみで、`selectionStart` が移動したときにのみ更新され、ビューによって更新されることはありませんでした。そこから2つの欠陥が生じました。ホイールジェスチャーは実際の要素を動かす一方で canvas はその場に留まるため、テキストがまったくスクロールしませんでした。さらに `selectionStart` は `value.length` で初期化されるため、マウントしたばかりの TextArea はコンテンツの**下端**を描画する一方でネイティブ要素は先頭に留まり——60行のドキュメントで 32.6 行のずれを実測——すべてのクリックのキャレットが誤った行に落ちていました。

キャレット追従は、ミラーが存在しない場合のフォールバックとして残されています。ミラーはさらに `scrollbar-width: none` を設定します：ネイティブのスクロールバーの溝が `clientWidth` を canvas の幅より狭めるため、両者は異なる位置で折り返します。2.9.0 の Firefox で実測したところ、幅 516px の TextArea には 12px の溝があり、ネイティブ要素は 480px で折り返す一方、canvas は 492px で折り返しました。
