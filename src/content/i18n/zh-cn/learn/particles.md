...
- **一次性标记粒子场。** 给 `ComputeParticleEntity`（或一个包装器）一个单一的 `getA11yAttributes()`，返回一个 `role` 和描述整个效果的 `aria-label`。一个节点，恒定成本。
- **在不投影的情况下进行命中测试。** `scene.findEntityAt(x, y)` 可以解析任何实体，无论其 `interactive` 状态如何，因此指针交互不需要投影元素。`pointerEvents: true` 向模拟提供光标坐标，与语义层无关。
- **如果效果纯粹是装饰性的，那么就这样说明。** 不进行投影是正确的做法，相当于装饰性DOM元素上的 `aria-hidden` —— 但要确保效果传递的任何*信息*也同样以文本形式提供。
