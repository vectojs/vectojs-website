...
**不要對每個粒子設定 `interactive = true`。** 這會為每個實體向語意層投影一個真實的DOM元素，而每個實體的成本會隨數量增長而惡化——在RTX 4060筆電上量測，20,000個獨立互動的移動實體在Chrome上耗時715ms/幀，在Firefox上耗時2,737ms/幀。參見[成本表](/learn/accessibility/#成本與互動實體數量成超線性關係)。
