...
Modèles autonomes pour les problèmes VectoJS les plus courants. Chaque recette est complète et copiable-collable.

---

## Boîte de dialogue modale

Rend une superposition bloquante au-dessus de tout le contenu de la scène. Se ferme au clic sur l'arrière-plan ou avec la touche Échap, et projette un repère `role="dialog"` pour les lecteurs d'écran.
...
> [!NOTE]
> La modale démarre à `y + 32` et s'anime jusqu'au centre vertical, offrant une entrée subtile de glissement vers le haut sans saut de mise en page. L'entité d'arrière-plan utilise `isPointInside(): true` afin que tout clic sur le voile (et non sur la carte modale au-dessus) aille à `close()`. L'écouteur keydown sur `document` est retiré immédiatement dans `close()` pour éviter les gestionnaires en double si `openModal` est appelée rapidement.

---

## Infobulle au survol

Attache une infobulle avec un délai de 400 ms à n'importe quelle entité. L'infobulle suit la position du pointeur signalée par l'événement `hover` et apparaît/disparaît en fondu sur la couche de superposition.
...
> [!NOTE]
> Le délai de 400 ms empêche les infobulles de clignoter lorsque le pointeur passe rapidement sur une entité. Ré-entrer dans la même entité avant que le minuteur ne se déclenche est une opération sans effet car le retour anticipé sur `timer !== null` empêche une double planification. Réinitialisez `timer` à `null` à l'intérieur de `setTimeout` si vous voulez autoriser un re-déclenchement pendant que l'infobulle est déjà visible.

---

## Glisser-déposer

Une entité `DraggableCard` capture directement les événements de pointeur sur le canvas (puisque `pointermove` doit se déclencher même lorsque le pointeur quitte les limites de l'entité). Les zones de dépôt se surlignent pendant que l'élément déplaçable les survole et le recentrent au relâchement.
...
> [!NOTE] > `canvas.setPointerCapture(e.pointerId)` maintient le routage de `pointermove`, `pointerup` et `pointercancel` vers le canvas même lorsque le pointeur quitte sa limite en plein glisser. Traitez `pointercancel` comme une annulation plutôt qu'une validation afin qu'une interruption du navigateur ne puisse pas laisser la carte bloquée dans l'état de glissement.

---

## Clic à l'extérieur pour fermer

Intercepte chaque clic en phase de capture sur la racine de la scène avant qu'il n'atteigne les enfants. Utilisé pour fermer les menus, dropdowns et popups lorsque l'utilisateur clique n'importe où en dehors d'eux.
...
> [!NOTE]
> L'option `{ capture: true }` sur `scene.getRoot().on()` est cruciale — sans elle, l'événement de clic est distribué en phase de propagation après avoir déjà été consommé par les enfants. En phase de capture, il se déclenche en premier, de sorte que vous pouvez fermer le popup avant que le clic n'atteigne toute entité en dessous.

---

## Graphique à barres animé

Chaque barre est une entité indépendante avec une propriété `displayHeight` que `animate()` pilote de 0 jusqu'à la hauteur cible de la barre. Les barres sont décalées avec `setTimeout` pour cascader de gauche à droite.
...
> [!NOTE] > `displayHeight` doit être une propriété numérique directe sur l'entité — et non imbriquée dans un tableau ou un objet — pour qu'`animate()` puisse l'interpoler. Le lissage est ease-out quadratique, ce qui donne une décélération naturelle qui fonctionne bien pour des barres qui grandissent.

---

## File d'attente de notifications toast

`ToastManager` maintient une file d'attente FIFO. Afficher un toast pendant qu'un autre est visible le met en file d'attente. Chaque toast apparaît en fondu, reste 3 secondes, puis disparaît en fondu avant que le suivant ne soit affiché.
...
> [!NOTE]
> L'indicateur `busy` verrouille `next()` afin qu'un seul toast s'exécute à la fois. Appeler `show()` pendant qu'un toast est affiché met le message en file d'attente — il apparaîtra après le toast actuel et tous les toasts précédemment mis en file d'attente. Si vous voulez des toasts parallèles empilés verticalement, suivez un `yOffset` qui s'incrémente par toast actif.

---

## Formulaire avec validation

Compose `Input`, `Slider`, `Toggle` et `Button` en un formulaire validé. Les erreurs sont des entités `Text` rendues en rouge et définies à la soumission ; elles s'effacent dès que l'utilisateur modifie le champ correspondant.
...
> [!NOTE]
> Les entités `Text` d'erreur sont toujours dans l'arbre de mise en page — elles affichent simplement une chaîne vide lorsqu'il n'y a aucune erreur. Cela garde la mise en page du `Stack` stable : aucun décalage lorsque les erreurs apparaissent. Si vous préférez masquer entièrement l'espace, passez à `entity.opacity = 0` et `entity.height = 0` lorsqu'il n'y a aucune erreur, puis restaurez les deux lorsqu'une erreur est définie.

---

## Impression nette via l'export SVG

Imprimer un `<canvas>` directement le rasterise : le navigateur met à l'échelle le bitmap au DPI de l'imprimante, donc le texte et les formes vectorielles sortent flous. `scene.toSVG()` capture l'état actuel de la scène via le `SVGRenderer` dans un document `<svg>` indépendant de la résolution — le moteur d'impression le rend ensuite en tant que vecteurs réels, nets à n'importe quel DPI. Aucune dépendance supplémentaire ; l'exporteur SVG est déjà inclus dans `@vectojs/core`.

```typescript
import { Scene } from '@vectojs/core';

/**
 * Imprime la scène actuelle en SVG vectoriel (net à n'importe quel DPI) plutôt
 * qu'en bitmap rasterisé du canvas. Ouvre le dialogue d'impression et nettoie
 * après.
 */
function printScene(scene: Scene): void {
  // toSVG() renvoie une chaîne <svg> complète et autonome avec un viewBox
  // correspondant à la largeur/hauteur de la scène — un instantané en lecture seule de l'état actuel.
  const svg = scene.toSVG();

  // Isole le job d'impression dans un iframe caché de même origine afin qu'il ne
  // perturbe pas le canvas actif et n'hérite pas de la feuille de style de l'écran.
  ...
  doc.open();
  // @page supprime les marges par défaut du navigateur ; width:100% permet au
  // vecteur de s'adapter à la feuille. Le SVG porte son propre viewBox, donc le
  // rapport d'aspect est préservé.
  doc.write(
  ...
  // Imprime une fois que le document de l'iframe s'est stabilisé, puis supprime
  // l'iframe que l'utilisateur imprime ou annule (afterprint se déclenche dans les deux cas).
...
> [!NOTE]
> `toSVG()` est un instantané de l'état _actuel_ de la scène, donc appelez-le au moment où vous souhaitez imprimer (par exemple, dans le gestionnaire de clic), et non une seule fois au démarrage. Il couvre la géométrie vectorielle, le texte et les images dessinés via le chemin du renderer standard ; les couches GPU uniquement (`WebGLPointRenderer` particules, `WebGPUParticleSystemManager`) ne font pas partie de la sérialisation SVG — pour celles-là, composez un rasterisé de secours. Étant donné que la sortie est du XML SVG brut, la même chaîne fonctionne également pour sauvegarder dans un fichier `.svg` (`new Blob([svg], { type: 'image/svg+xml' })`) ou pour le rendu côté serveur.
