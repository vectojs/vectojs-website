...
- 도크(기본값: `position: fixed; right: 0; width: 320px`, 뷰포트 전체 높이)와 그 캔버스는 `pointer-events: none`이며, 이는 메인 `Scene`의 `a11yRoot`가 옵트아웃하고 개별 대화형 그림자 요소가 `auto`(`@vectojs/devtools@0.4.3+`)로 옵트인하는 방식을 미러링합니다. 이는 도크의 빈 배경/크롬 위의 클릭이 아래에 있는 호스트 콘텐츠로 통과됨을 의미합니다 — 호스트 앱의 오른쪽 가장자리 컨트롤(탭 닫기 버튼, 툴바 버튼)을 포함하며, 그렇지 않으면 도크의 320px 대역 아래에 가려집니다. 패널 자체의 a11y-프로젝션된 컨트롤(버튼, VMT 트리 행)만 자체 `auto` 옵트인을 통해 독립적으로 클릭 가능합니다.
