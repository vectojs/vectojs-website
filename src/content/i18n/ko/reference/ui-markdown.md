...
<figure class="sandbox component-demo">
  ...
  <iframe src="/sandbox/ui/markdown.html?v=core-1.31.0-ui-2.13.0" class="sandbox-frame component-demo-frame component-demo-frame-xl" loading="eager" title="Markdown 라이브 데모" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
...
디스플레이 수식은 의도적으로 자체 너비를 유지합니다: MathJax는 조판 박스 크기를 사용 가능한 너비가 아니라 `ex` 기준 메트릭으로 정하므로, 늘리면 수식이 왜곡됩니다. 펜스 코드도 다시 줄바꿈되지 않습니다 — 코드는 고정 고정폭 그리드를 가지며 긴 줄은 설계상 넘칩니다 — 배경만 크기가 조정됩니다.
...
| `$…$` / ` ```math ` | MathJax로 조판된 수식(인라인 / 블록). 구분자가 닫힌 후에만 변환됩니다                                     |
