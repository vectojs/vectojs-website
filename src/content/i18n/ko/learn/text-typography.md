...
`TextEntity`는 둘 다 직접 제공합니다: `text.setTextAlign('justify')`, `text.setHyphenator(fn)` — 자세한 내용은 [코어 API 참조](/reference/core-api/#textentity--gridtextentity-from-)를 확인하세요. `TextEntity`가 각 글리프를 자체 계산된 위치에 그리므로 올바르게 렌더링됩니다. `@vectojs/ui`의 `Text`/`RichText` 컴포넌트는 성능을 위해 각 래핑된 줄을 단일 네이티브 `fillText()` 호출로 축소하므로 아직 글리프별 양쪽 정렬을 지원하지 않습니다 — 양쪽 정렬된 본문 텍스트가 필요하면 `TextEntity`를 사용하세요.
